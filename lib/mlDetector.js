...
// Suppress warnings like "Unknown CPU vendor"
...
export class MLDetector {
    ...
    parseYoloOutput(data, shape, confThreshold, scaleX, scaleY) {
        ...
        for (let i = 0; i < numAnchors; i++) {
            ...
            // Special handling for YOLOv8-seg (QR) which has 37 channels:
            // 4 box + 1 class + 32 mask.
            // We must ONLY look at the class score (index 4) and ignore the masks (indices 5-36).
            const isSegmentation = (shape[1] === 37);

            if (isSegmentation) {
                // Single class QR detection
                maxScore = data[4 * stride + i];
                bestClass = 0;
            } else {
                // Standard detection logic for other models
